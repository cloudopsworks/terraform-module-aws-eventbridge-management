  ...
  targets = merge(
    ...
    [
      for rule_key, rule in local.rules : {
        for target in try(rule.config.targets[*], []) : format("%s-%s", rule_key, target.target_id) => {
          rule_key    = rule_key
...
resource "aws_cloudwatch_event_bus" "this" {
  ...
  dynamic "dead_letter_config" {
    for_each = try(each.value.config.dead_letter_sqs, null) != null && try(each.value.config.dead_letter_sqs, "") != "" ? [1] : []
    content {
      arn = try(each.value.config.dead_letter_sqs, null)
    }
  }
  log_config {
    include_detail = try(each.value.config.logs.include_detail, null) != null && try(each.value.config.logs.include_detail, "") != "" ? each.value.config.logs.include_detail : "FULL"
    level          = try(each.value.config.logs.level, null) != null && try(each.value.config.logs.level, "") != "" ? each.value.config.logs.level : "ERROR"
  }
  tags = merge(
    local.all_tags,
...
resource "aws_cloudwatch_event_rule" "this" {
  ...
  event_bus_name      = try(each.value.config.event_bus_ref, null) != null && try(each.value.config.event_bus_ref, "") != "" ? aws_cloudwatch_event_bus.this[each.value.config.event_bus_ref].name : "default"
  event_pattern       = try(tostring(each.value.config.event_pattern), jsonencode(each.value.config.event_pattern), null)
